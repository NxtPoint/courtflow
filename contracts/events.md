...
| `lesson_proposed` | a coach counter-proposes a new time on a client's request (awaiting the client; on-behalf bookings auto-confirm and do NOT propose) | `ref_type=booking`, `ref_id`, `resource_name`, `starts_at`, `ends_at`, `coach_name?` | **Client: accept / decline** | **transactional** |
