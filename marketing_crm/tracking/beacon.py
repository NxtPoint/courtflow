...
#
# SHARED ENGINE (replicable across sites — kept in lock-step with the 1050/ten-fifty5 repo). The
# metadata contract is identical everywhere so the analytics aggregation is portable:
...
# the Cloudflare edge header; device/browser/os are parsed server-side from the User-Agent.
#
# page_view is intentionally NOT forwarded to Klaviyo (would be noisy/expensive) — DB only.
# Multi-tenant: club_id is carried into the event.
...
def _parse_ua(ua):
    ...
    the 1050/ten-fifty5 repo so device analytics are identical across sites."""
    u = (ua or "").lower()
...
def _record(path, club_id, email, referrer, ctx):
    try:
        ...
        with session_scope() as s:
            ...
            if ctx.get("is_leave"):
                ...
                usage_events.record_usage(
                    s, event_type="page_leave", club_id=club_id, account_id=account_id,
