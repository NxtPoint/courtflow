...
- `test_booking_scenarios.py` · `test_billing_scenarios.py` · `test_statement_reconciliation.py`
  — rollback-only scratch-DB harnesses (**booking 371 / billing 492 / statement 64**).
...
- `migrate_lesson_requests.py` — one-off: clear the lesson requests left over from the approval gate (removed 2026-07-29 — one lesson flow). No NEW `requested`/`proposed` lesson can be created, but the ones made while it gated are live and accept/propose/decline stay alive only to finish them. Per request: **PAID → accept** (honour it; reuses the existing order, never a second charge) · **in the PAST → cancel** (nobody is turning up, and it sits on the coach's queue forever) · **unpaid FUTURE → left for a human** (reported, never guessed). **Dry-run by default**; `--commit` to act; `--club` to scope. Re-runnable. Once it reports nothing pending, the approval path + its 4 email templates + the `requested`/`proposed` statuses can be deleted.
