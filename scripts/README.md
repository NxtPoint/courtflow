...
| "I booked but never got the confirmation email" | `diagnose_email.py` | separates *we never tried* / *SES refused us* / *it sent and went to spam* |
| "they paid but it still shows owed" / "this booking isn't on their record" | `diagnose_person_money.py <name>` | every order, line, payment, booking (incl. cancelled) + the coach split |
| "this owed amount opens a PAID record and I can't act on it" | `audit_duplicate_heads.py` | lists lessons billed to more heads than played; voids only ids you name |
| "the class won't move to the new time" | `diagnose_class_move.py "<class>" --to 17:15` | per week: would it move, and if not WHAT is in the way |
| "email was down — who never got their receipt?" | `resend_failed_emails.py` | dry-run lists them; `--commit` + typed YES resends the MONEY documents only |
...
- `test_booking_scenarios.py` · `test_billing_scenarios.py` · `test_statement_reconciliation.py`
  — rollback-only scratch-DB harnesses (**booking 700 / billing 728 / statement 64**).
...
- `diagnose_signup_optin.py` — READ-ONLY: **why is a new signup still landing opted OUT when `club.policy.marketing_opt_in_default` is TRUE?** Every piece of that path works when called on its own, so the failure is on the live path and HIDDEN — the block in `auth/principal.py` that sets the flag sits inside `try: ... except Exception: log.debug(...)`, and an exception anywhere in it skips the flag silently. This prints, per signup day, what actually got written: opt-in on `iam.user`, opt-in on the `core.app_user` gate, whether an app_user row exists at all, whether the `core.person` satellite was created, and whether the free-week trial was granted. The trial is the control — it fires on the SAME `_created` gate a few lines earlier, so *trial YES + person NO + optin NO* proves the gate fired and the block threw, while *trial YES + person YES + optin NO* means the block ran and the policy read came back False for the wrong club. It also lists every `club.policy` row in the database, because the code reads the policy for the club resolved from the MEMBERSHIP and a missing row there quietly means "opt out". Prints a verdict naming which of those shapes it sees. `--days N` (default 21).
- `diagnose_email.py` — READ-ONLY: **why did the transactional emails stop?** Every send in this platform is best-effort and silent by design — `ses.enabled()` self-gates on creds, `send_email` catches everything and returns False, and `_try_email` turns both into a status string — so a member must never lose a booking because SES is down. The cost of that design is that a revoked IAM key, a deleted SES identity, a dropped env var and an account pushed back into the SES sandbox all present IDENTICALLY: bookings keep working, the confirmation simply never arrives, and nothing is logged where anyone looks. This asks the three questions that separate them, in the order that narrows fastest — **A. does the app still SEE a sender + credentials** (`ses.enabled()`, which vars are unset), **B. do those credentials still WORK** (send quota, account-sending, identity + DKIM verification, each probe reported separately because a narrowed policy can deny the probe while still allowing the send), **C. what does `core.notification.email_status` actually say, per day** — `sent` worked, `failed` means SES refused us, `skipped` means we never tried. **The day that column flips is the day the change landed**, which is the fact that identifies the cause. It also prints `diary.reminder_log`'s last entry, because the hourly reminder sweep is a GitHub Action whose `curl` ends in `|| echo`: a rotated `OPS_KEY` stops every reminder with a **green** workflow, and that looks identical from a member's inbox. Ends with a verdict naming which of the three faults it is — including the one this codebase cannot fix, *SES accepted everything and it went to spam*, which is a DMARC/SPF/DKIM question. Never prints a credential: the access key ID appears as its last four characters only (enough to tell a rotated key from a removed one against the IAM console) and any AKIA-shaped string inside an AWS error is masked. `--days N` (default 21) · `--no-db` for config+AWS only · `--to you@example.com` sends exactly one real test email, the only thing here that is not read-only.
- `resend_failed_emails.py` — DRY-RUN-DEFAULT: send the money documents an email outage swallowed. A failed transactional send is a **dead end, not a queue** — `email_status='failed'` is written and nothing ever retries it — so when the SES credential died on 2026-09-02 (see `diagnose_email.py`) 324 emails were rendered, attempted, refused and abandoned. **Most of them should stay unsent:** "your booking is confirmed" for a court played last Thursday is a system announcing its own fault to someone who has moved on. What has to arrive late is the MONEY — receipts, refunds, memberships, packs, invoices — because those are documents people keep, reconcile and occasionally need in a dispute, and their value does not expire. So this resends exactly that set (`MONEY_KINDS`) and nothing else. It renders through `notifications._try_email`, the SAME renderer the live path uses, and re-loads the rich detail block live exactly as `deliver()` does while reusing the STORED title/body — a resent receipt must be the email that should have arrived, not a fresh template run against data that has since moved. A "this is arriving late" line is prepended by default (`--no-notice` to drop it), because without it the member's reasonable reading is *they have just charged me again*. **`refund_requested` is deliberately held back and merely LISTED** — an automated "we'll be in touch" seven days on is worse than the silence it replaces, so those people get a human. Idempotent: only `failed` rows are picked up and a success flips them to `sent`, so nobody is emailed twice and a re-failure is retried next run. Refuses to start if SES is unconfigured rather than grinding out N no-ops. `--commit` + a typed **YES** to send (it is real mail and cannot be un-sent); `--limit N` for a cautious first batch; `--since`, `--kinds`, `--sleep`.
- `diagnose_person_money.py` — READ-ONLY: **one person's complete money trail, including what the Client-360 record deliberately hides.** Two symptoms get reported repeatedly and they are the same shape — *the screen and the money disagree about this person* — and neither can be settled from a screen (the screen is what's under suspicion) or from one table (a debt, the thing it was for, the cash and the coach's share are four rows that can each be right while the set is wrong). **S1 "a cancelled booking isn't on their record but its money is in Sales by day":** `client360._bookings` filters `bk.status <> 'cancelled'` UNCONDITIONALLY while Sales-by-day counts `billing.payment` rows, which know nothing about booking status. When a cancel voids or refunds, the two agree; when a cancellation KEEPS the money, that payment is real revenue the client's own record cannot account for — the **INVISIBLE MONEY** section names it, with the amount, and is the reason this script exists. **S2 "they paid, I marked it paid, it still shows owed":** almost always a SECOND debt, because `record_desk_payment` refuses anything but an exact amount on an `open`/`awaiting_payment` order, so a payment that was accepted DID close the order it named — the one still owed is a duplicate order, a 'Pay all' wrapper whose children weren't settled (or a child whose wrapper was), or a per-head semi-private order. Printing every order beside its lines and payments makes that visible in one look. It also answers the question that always follows — *did the coach get paid on it?* — with the `billing.commission_split` per paid coaching line (`reconcile_coach_commission` proves it club-wide; this proves it for the person in front of you). The CLUB comes from the person who matched, never first-by-`created_at`. `<needle>` matches email/first name/surname partially; `--month YYYY-MM`.
- `audit_duplicate_heads.py` — DRY-RUN-DEFAULT: **lessons billed to more heads than actually played.** A lesson may legitimately carry several orders — SEMI-PRIVATE bills PER HEAD, so a squad of three raises three real debts — and the same machinery misfires when a lesson is booked with the PAYER named as the client and the PLAYER added as an extra: two heads billed for one person on court. Live example (2026-09-09): a parent and a child, orders created in the SAME SECOND on each of three lessons, the parent's settled by a R550 'Pay all' and the child's left open for ever — money banked once, coach accrued once, and a duplicate receivable that could not be acted on because opening it from the client's record lands on the PAID order, which offers nothing to do. **It voids nothing on its own judgement:** the fact that separates a phantom head from a real squad member is who was ON COURT, which is a fact about the club's day and not about the rows, and guessing wrong ERASES A REAL DEBT silently and in the direction that costs the club money. So it reports (flagging same-second creation, which proves one booking action raised them all), a human decides, and `--void-ids` acts only on full ids you type. It refuses, whatever you type: an order that has taken any money, an order that is not `open`/`awaiting_payment`, and an order with no PAID sibling at the same amount on the same booking. Voiding an unpaid duplicate moves no money and touches no commission — that accrued on the paid sibling. **Not `void_orphaned_orders`**, which requires every booking on the order to be cancelled and so can never touch a completed lesson.
- `diagnose_class_move.py` — READ-ONLY: **why won't this class move to that time, week by week?** `reschedule_series` refuses an occurrence it cannot legally move and reports a count and a code — honest, and unusable: it names neither the date nor the thing in the way, so the only recourse is to hunt through the diary. Several blocked weeks in a row then read as *"it did nothing from the 8th onwards"*, which is a completely different complaint from the truth. This answers the real question per occurrence WITHOUT MOVING ANYTHING: for each upcoming session, would it move to `--to HH:MM`, and if not, **what** is in the way — the coach's own other class or lesson and its time, or no free court. It calls the SAME guards the mover calls (`_coach_busy_at`, plus a court check mirroring `_reserve_courts_for_class`, excluding the session's own holds so a class is never reported as blocked by itself), so a green line here means the move will actually take — a second implementation of the guards would only tell you what different code thinks. Run with no `--to` to see what is scheduled now.
