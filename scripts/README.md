...
- `audit_marketing_reach.py` — READ-ONLY: how many people we HAVE vs how many we may legally EMAIL, and whether the 7-day trials ever converted. A headcount is not an audience — on live data 1252 members yielded 455 reachable, and of 368 trials only 4 (1.1%) ever bought a membership while 93% did nothing at all after the trial ended, which turns an "upsell" campaign into a reactivation one. Prints BOTH marketing_opt_in flags (they disagree: iam.user vs the core.app_user Klaviyo gate) and three ready-made campaign segments already filtered to opted-in-and-emailable. No --commit; every statement is a SELECT, so it is safe on the Render Shell. `--club <name>` when several exist.
