...
#
# Endpoints (read-only):
#   GET /api/analytics/overview?days=30&club_id=<uuid?>  — the dashboard payload for THIS platform.
#       platform_admin: all clubs, or one via ?club_id. club_admin: forced to their own club.
#   GET /api/analytics/clubs  — platform_admin only: clubs for the filter dropdown.
#
# Per-business by design: each app shows its OWN overview (no cross-business/consolidated view —
# the Ten-Fifty5 "bridge" was deprecated 2026-06-21; Ten-Fifty5 has its own /backoffice cockpit).
#
# Auth: auth.principal.resolve_principal; gate via iam.permissions.can('view_club_analytics').
# Aggregation is in analytics.repositories (guarded SELECTs — a missing/empty table yields empty
# panels, not 500s).
