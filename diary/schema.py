...
_DDL = [
    ...
    # column CHECK so existing DBs match; boot-twice safe.
    #
    # 'requested'/'proposed' were RETIRED with the lesson approval gate (2026-07-29 — one lesson
    # flow). Narrowing the CHECK is guarded on there being no such rows LEFT: this runs on every
    # boot, and an ALTER that fails takes the whole deploy with it. So if a stray request ever
    # exists, the constraint simply stays wide and nothing breaks — the code no longer writes those
    # values either way, and scripts/migrate_lesson_requests.py clears them.
    f"""
    DO $$
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM {SCHEMA}.booking
                        WHERE status IN ('requested','proposed')) THEN
            ALTER TABLE {SCHEMA}.booking DROP CONSTRAINT IF EXISTS booking_status_check;
            ALTER TABLE {SCHEMA}.booking ADD CONSTRAINT booking_status_check
