...
| D4 | **NEW CourtFlow Clerk app** (`settling-alien-23.clerk.accounts.dev`, dev instance) — reverted to `docs/04 §2`'s original recommendation after a brief attempt to reuse 1050's Clerk failed: the 1050 *production* instance is domain-locked to ten-fifty5 hosts and rejected the `onrender.com` origin. A **dev instance** (`pk_test_…`) works on any origin with no domain config — ideal until `nextpointtennis.com` cuts over to a production instance. Session token customized with an `email` claim. `AUTH_JWKS_URL`/`AUTH_ISSUER`/`CLERK_PUBLISHABLE_KEY` committed inline in `render.yaml`. Access still gated by `iam.membership`. | Tomo 2026-06-20 |
