...
| `HOME_CLUB_SLUG` (courtflow-api) | ⚪ unset = `nextpoint` | The club the platform's one-per-platform env settings belong to (`club/home.py`): `TRANSACTIONAL_BCC`, the Klaviyo account and `SIGNUP_TRIAL_DAYS` apply to THIS club only — never to another club's customers. Another club's free week is `club.policy.signup_trial_days`. | a club slug |
