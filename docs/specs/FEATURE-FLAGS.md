...
|---|---------|-----------|---------|-------|---------|
| ~~A1~~ | ~~**Klaviyo** marketing sync~~ — **DONE 2026-08.** Key set; ~506 subscribers on *NextPoint Members* (single opt-in); trial + campaign flows sending. The gate is `core.app_user.marketing_opt_in`, not `iam.user`. Audit with `scripts/audit_marketing_reach.py`. | — | — | — | The two cohort scripts (`scripts/klaviyo_reactivation.py`, `scripts/klaviyo_trial_cohort.py`) remain **manual**. |
