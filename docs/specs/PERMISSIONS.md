...
| **Join a game** | any member — but **ONE PERSON, ONE PLACE**: refused if they already hold a seat in an overlapping game (`ALREADY_PLAYING_THEN`), and a COACH is refused while the diary has them teaching (`COACH_IS_WORKING`). A seat is a `booking_party` row, not a resource booking, so the GiST constraint cannot see it — joining bypassed the commitment check that `create_booking` has always run. Both are REFUSALS rather than price downgrades: being in two places at once is not something a fee can fix. |
