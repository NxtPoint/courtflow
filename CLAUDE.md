...
## How to talk to Tomo (read this first)

Tomo is the owner/operator, not a full-time engineer. This section outranks every
formatting habit further down this file.

**Every reply.** Lead with the answer in 1-2 plain sentences, then the next action, then
STOP and offer the detail. No preamble, no restating the question. Long replies bury the
answer and he stops reading.

**Jargon.** Keep it out of the opening lines. If a term is unavoidable, define it in half a
sentence the first time you use it ("SERVFAIL - the DNS server refused to answer"). Depth is
welcome once he asks for it.

**Say what it means, not just what it is.** Name the part of the product affected and the
consequence for a customer or for the business, before you name the file, function or library.

**Decisions - the important one.** Never hand him a menu of technical options. When you need
a call, give him, in this order:
1. the decision in one plain sentence, framed as a product or business choice, not a technical one;
2. your recommendation, and why, in one sentence;
3. the trade-off - what he gives up by taking it;
4. what it costs to undo if it turns out wrong.
If he cannot make the call without first understanding an internal, that means it has not been
framed as a business choice yet. Reframe it; do not explain the internal and ask again.

**Do not ask at all when you do not have to.** Do the obvious thing and say what you did.
Interrupt him only for choices that are expensive, hard to reverse, or that change what the
product does for a customer. Batch everything else to the end of the task.

**Exceptions that still lead, however long they run:** a warning that stops him doing something
harmful, a correction of something you got wrong, or a risk he is walking into. Brevity never
means withholding those.

**Format.** Short lists and small tables read well; walls of prose do not. Never manufacture
confidence - "I don't know, and here is how we would find out" is a good answer.

*(Set 2026-09-07. Supersedes any earlier note on how to answer Tomo.)*
...
5. `python -m scripts.test_all` — the JS parse gate (first, no DB) then three rollback-only
   ...
   **booking 700 / billing 735 / statement 64**. Each uses its own scratch club and always rolls back.
   Run one lane's harness standalone while iterating (each needs `DATABASE_URL` = a local sandbox):
   `python -m scripts.test_booking_scenarios` (diary) · `python -m scripts.test_billing_scenarios` (billing) ·
   `python -m scripts.test_statement_reconciliation`.
   **There is no per-test filter** — each harness runs its whole `SCENARIOS` list (93/100/12 `sc_*`
...
- A MONEY GATE THE CALLER HAS TO REMEMBER IS A GATE IN THE WRONG PLACE — `apply_seat_orders` left the
  `seat_rule_enforced` check to its callers and 3 of 4 forgot, so a member was billed a share with
  charging switched OFF (found live). The gate is now INSIDE it —
